import { DataLoader, Variable } from "@microsoft/chartifact-schema";
import { validateDataSource } from "./data.js";

export async function validateDataLoader(dataLoader: DataLoader, variables: Variable[], otherDataLoaders: DataLoader[]) {
    const errors: string[] = [];
    ...
    switch (dataLoader.type) {
        case 'file': {
            errors.push(...validateDataSource(dataLoader, variables, otherDataLoaders.filter(dl => dl.type !== 'spec')));
