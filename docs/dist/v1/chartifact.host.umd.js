      ...
      if (typeof element === "string") {
        ...
        switch (element.type) {
          ...
          case "dsv": {
            ...
            const dsvContent2 = Array.isArray(content) ? content.join("\n") : content;
            mdElements.push(tickWrap(`dsv delimiter:${delimiter} ${variableId}`, dsvContent2));
            break;
          }
          case "json-value": {
            const { variableId, content } = element;
          ...
          case "yaml-value": {
            ...
            if (typeof content === "string") {
              ...
              yamlContent = dump(content, { indent: defaultJsonIndent });
            }
...
`;
  ...
  async function resolveUrl(title, base, relativeOrAbsolute, host) {
    ...
    if (result.idoc) {
      await host.render(title, void 0, result.idoc, false);
    } else if (result.markdown) {
      await host.render(title, result.markdown, void 0, false);
    } else if (result.folder) {
      await host.render("Error", "Nested folders are not supported", void 0, false);
    } else {
      host.errorHandler(
  ...
  async function determineContent(urlOrTitle, content, host, handle, showRestart) {
    ...
    if (handle) {
      if (result.error) {
        ...
        await host.render(urlOrTitle, void 0, result.idoc, showRestart);
      } else if (result.folder) {
        loadFolder(urlOrTitle, result.folder, host);
      } else if (result.markdown) {
        await host.render(urlOrTitle, result.markdown, void 0, showRestart);
      }
    }
    return result;
  }
